...
const MAG_FACTOR: f32 = 0.6;        // How much branches shrink per iter
...
impl Tree {
    ...
    pub fn generate_new_sub_trees(&mut self, n: usize, angle: f32) {
        if self.has_sub_trees() {   // Traverse down tree
            for sub_tree in self.sub_trees.iter_mut() {
                sub_tree.generate_new_sub_trees(n, angle);         // Recursively call function for each sub tree
            }
            ...
            let new_magnitude = my_magnitude * MAG_FACTOR;
